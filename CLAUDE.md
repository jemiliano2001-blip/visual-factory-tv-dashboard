...
Model IDs referenced as string literals: text tasks and voice-command understanding use `gemini-3.7-flash` (updated 2026-08-20 from `gemini-3.5-flash` — per Google's own model docs, 3.5 is now the "legacy... baseline" tier while 3.7 is "latest and most capable... for complex coding, agentic workflows, and reliable multi-step execution," which fits the structured-JSON voice/filter/planning calls here better; verified callable with a live `generateContent` request against the project's key before switching, since a past incident shipped a documented-but-uncallable model id). `gemini-3.5-flash` stays in the allowlist as a rollback path. **TTS (`generateSpeech`) must use a dedicated audio model — `gemini-3.1-flash-tts-preview`** (newer Flash TTS: expressive, steerable via prompt, streaming-capable) — unaffected by the 3.7 bump. Do not use `gemini-2.5-flash-preview-tts` unless rolling back. A text flash model (3.5, 3.6, or 3.7) does NOT emit audio (`inlineData` comes back empty → silent response), so never swap the TTS model for one of them in a bulk model bump. `gemini-2.0-flash` is **deprecated — do not reintroduce it**. The allowlist (`ALLOWED_AI_MODELS`) lives in `shared/geminiProxy.ts`, imported by both `server.ts` and `functions/src/index.ts` — update it there once, not in each entry point separately. **This allowlist is baked into the deployed Cloud Function**: bumping the frontend's model string without redeploying `functions` makes every AI call fail with `Modelo no permitido` in production, silently (no build/test failure catches it) — always deploy functions and hosting together after a model change.

**Voice command pipeline** (`TVDashboard.tsx`, on every Web Speech API final transcript): `tryLocalFastVoiceCommand()` (`src/services/ai.ts`) runs first — pure regex/pattern matching against the transcript, no network call, resolves in under 5ms. Precedence matters: it first bails out to `null` on interrogative phrasing (`qué | cuánto | cuál | hay | ...` — a question needs Gemini to answer it, not a filter), then tries "limpiar filtros", then a direct PO lookup (digits or Spanish number words like "quinientos cuarenta y seis" via `parseSpanishNumberWords()`, matched against the trailing digits of `o.name` so a leading year never collides), and only then client/status filtering — client name and status (`overdue | delivered | pending | critical`) are combinable in one utterance ("las de Nissan que están vencidas" filters both at once). It returns `null` when nothing matches, and only then does the code fall through to `processTextVoiceCommand()`, which sends the transcript to Gemini (`gemini-3.7-flash`, 10s timeout) and returns a JSON action (`highlight | filter | answer`) plus a Spanish `message`, optional `filter_type` (`all | overdue | pending | delivered | critical`) / `filter_client`, and an optional `expected_order` (`ExpectedOrderInfo`) describing the order the operator likely means — rendered by the `VoiceFeedbackOverlay.tsx` HUD. `processVoiceCommand()` (audio inline) remains available but is not the TV default. `TVDashboard.tsx` sets `recognition.maxAlternatives = 3` and runs the fast path over every alternative transcript (picking a `highlight` result over a `filter` result if both appear) to raise the odds of matching a PO number despite shop-floor noise. **When adding a new voice intent, add the deterministic pattern to `tryLocalFastVoiceCommand()` first** if it's a common/fixed phrasing — only intents that need real interpretation should fall through to Gemini. Caution: JS regex `\b` does not match after an accented vowel (`/qué\b/` never matches "qué "), use a `(?=\s|$)` lookahead instead for Spanish word boundaries.

TTS is **not** split by path — `generateSpeech()` is the voice for every command response (local match or Gemini), fired as soon as `result.message` is known. This is deliberate: the app already depends on the network for everything else (Odoo polling, Gemini interpretation), so avoiding a network call for voice specifically buys nothing, and Gemini's voice is clearly better than the browser's native one. It sends a short Spanish style prompt + the message to `gemini-3.1-flash-tts-preview` with voice **Sulafat** (Warm) with a short timeout and no retries (`generateContent(..., 12000, 0)` — don't let a TTS hiccup stall the operator), and returns raw **PCM audio at 24 kHz** encoded as base64. `playPCMBase64()` in `TVDashboard.tsx` decodes it manually (16-bit little-endian samples → Float32) and plays it via a singleton `AudioContext` (`sharedAudioCtx`, `sampleRate: 24000`), tracking the currently-playing `AudioBufferSourceNode` in a module-level `activeAudioSource` so a new mic press can interrupt it (`stopSpokenAudio()`). Don't replace this with `<audio src="data:...">` — browsers won't decode raw PCM without a WAV header. `speakFastLocal()` (browser-native `window.speechSynthesis`, `es-MX`) is only an **emergency fallback** if `generateSpeech()` throws or returns no audio — not a latency optimization; don't reintroduce it as the default path.
